...
export default class P2PNetwork {
    ...
    async sendTransaction(tx) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.send({ type: 'TX', tx: CryptoUtils.serializeWithBigInt(tx)})
                .catch(err => {
                    clearTimeout(timer);
                    reject(err);
                });
    ...
    async getBlock(height) {
        ...
        for (let i = 0; i < consts.MAX_RETRIES; i++) {
            try {
                const blockData = CryptoUtils.deserializeWithBigInt(await this._requestWithTimeout('GETBLK', { height: Number(height) }, consts.REQUEST_TIMEOUT));
                
                const block = new Block(
                    blockData.header.index,
                    blockData.header.prevHash,
                    blockData.body.map(tx => new Transaction(tx)),
                    blockData.header.validator,
                    blockData.header.stateRoot,
                    blockData.header.signature,
